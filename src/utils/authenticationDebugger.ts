...
export class AuthenticationDebugger {
  ...
  static async diagnoseUser(userId: string, email: string): Promise<AuthDebugReport> {
    console.log('🔍 AuthenticationDebugger: Diagnosing user authentication')
  ...
  static async testUserAuthentication(email: string, password: string): Promise<{
    ...
    console.log('🧪 AuthenticationDebugger: Testing user authentication')
    ...
    try {
      ...
      result.details.push(`Password verification: ${decryptedPassword === password ? 'successful' : 'failed'}`)
  ...
  static async resetUserPassword(userId: string, newPassword: string): Promise<{
    ...
    console.log('🔄 AuthenticationDebugger: Resetting user password')
  ...
  private static async fixDemoUserCredentials(result: any): Promise<void> {
    ...
    ] // Note: These are hardcoded for demo user creation only
    ...
    for (const userConfig of demoUserEmails) {
      try {
        ...
        if (userResponse.status === 'success' && userResponse.data) {
          ...
          if (!credentials) {
            console.log('🔧 Creating missing credentials for system user')
            const resetResult = await this.resetUserPassword(actualUserId, userConfig.password)
            if (resetResult.success) {
              result.issuesFixed.push('Created missing credentials for system user')
            } else {
              result.errors.push('Failed to create credentials for system user')
            }
          }
        } else {
          result.errors.push('System user not found in system')
        }
      } catch (error: any) {
        result.errors.push(`Error fixing system user: ${error.message}`)
      }
    }
  }
}
