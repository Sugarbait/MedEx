...
export const MFASetup: React.FC<MFASetupProps> = ({ user, onComplete, onCancel }) => {
  ...
  const generateMFASecretViaMFAService = async () => {
    try {
      console.log('Generating MFA secret using MFA service')
  ...
  const verifyCode = async () => {
    ...
    try {
      ...
      if (verificationResult.success) {
        ...
        console.log('❌ MFA verification failed via service')
        ...
        {step === 'setup' && (
          <div className="space-y-6">
            ...
            <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
              ...
              <div className="bg-white dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700 p-3 flex items-center justify-between">
                <code className="text-sm font-mono text-gray-900 dark:text-gray-100">
                  {formatSecret(secret)}
                </code>
                <button
                  onClick={copySecretToClipboard}
                  className="ml-2 p-1 text-gray-500 hover:text-gray-700 dark:text-gray-300"
            ...
            <div className="flex gap-3">
              <button
                onClick={onCancel}
                className="flex-1 px-4 py-2 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-50 dark:bg-gray-700"
              >
                Cancel
              </button>
              <button
          ...
          <div className="space-y-6">
            ...
            <div>
              <input
                ...
                className="w-full px-4 py-3 text-center text-2xl font-mono border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                maxLength={6}
                autoComplete="off"
              />
            </div>
            ...
            <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                {backupCodes.map((code, index) => (
                  <div key={index} className="bg-white dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700 p-2 font-mono text-sm text-center">
                    {code}
                  </div>
                ))}
              </div>
