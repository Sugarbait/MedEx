...
export const encryptionConfig = {
  phiKey: import.meta.env.VITE_PHI_ENCRYPTION_KEY || 'default-development-key-not-for-production-32chars',
  auditKey: import.meta.env.VITE_AUDIT_ENCRYPTION_KEY || 'default-development-audit-key-not-for-production-32',
  algorithm: 'AES-256-GCM'
}

// Temporarily disable encryption key requirement for development
// if (hipaaConfig.encryptionEnabled && (!encryptionConfig.phiKey || !encryptionConfig.auditKey)) {
//   throw new Error('PHI encryption keys are required when HIPAA mode is enabled.')
// }