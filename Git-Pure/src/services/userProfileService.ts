...
export class UserProfileService {
  ...
  static async loadUserProfile(userId: string): Promise<ServiceResponse<UserProfileData | null>> {
    try {
      ...
      console.log('UserProfileService: Loading user profile for userId:', userId)
      ...
      // Since Supabase functions don't exist, fall back to localStorage
      ...
      if (storedUser) {
        try {
          ...
          if (userData.id === userId) {
            ...
            console.log('UserProfileService: Successfully loaded user from localStorage:', userData.email)
  ...
  static async saveUserProfile(userProfileData: UserProfileData): Promise<ServiceResponse<UserProfileData>> {
    try {
      ...
      if (userIndex >= 0) {
        ...
        console.log('UserProfileService: Updated existing user in systemUsers:', userId, userForStorage.name)
      } else {
        usersList.push(userForStorage)
        console.log('UserProfileService: Added new user to systemUsers:', userId, userForStorage.name)
      }

      localStorage.setItem('systemUsers', JSON.stringify(usersList))
      console.log('UserProfileService: User saved to localStorage:', userId)
    ...
    try {
      ...
      defaultDemoUsers.forEach(demoUser => {
        ...
        if (hasBeenDeletedById || hasBeenDeletedByEmail) {
          console.log('Skipping permanently deleted demo user:', demoUser.name, 'Reason:', hasBeenDeletedById ? 'ID deleted' : 'Email deleted')
          return
        }
        ...
        if (existingIndexById === -1) {
          ...
          if (existingIndexByEmail === -1) {
            console.log('Adding missing demo user:', demoUser.name)
            users.push(demoUser)
          } else {
            console.log('Demo user email already exists, skipping:', demoUser.email)
          }
        } else {
          ...
          console.log('Preserving existing demo user changes:', existingUser.name)
        }
      })
      ...
      const usersWithAvatars = await Promise.all(users.map(async (user: any) => {
        try {
          ...
          if (avatarUrl) {
            console.log('UserProfileService: Found avatar for user:', user.name, avatarUrl)
            return { ...user, avatar: avatarUrl }
          } else {
            console.log('UserProfileService: No avatar found for user:', user.name)
          }
        } catch (error) {
  ...
  static async deleteUser(userId: string): Promise<ServiceResponse<void>> {
    try {
      ...
      if (userEmail && !deletedEmailList.includes(userEmail.toLowerCase())) {
        ...
        console.log('UserProfileService: Added email to deleted list:', userEmail)
      }
    ...
    try {
      ...
      for (const user of users) {
        ...
        if (!seenEmails.has(emailKey)) {
          ...
          removedCount++
          console.log(`Removing duplicate user: ${user.name} (${user.email})`, user.id)
        }
      }
