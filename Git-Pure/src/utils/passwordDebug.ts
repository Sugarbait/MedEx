...
export class PasswordDebugger {
  ...
  static clearUserCredentials(userId: string, email?: string) {
    ...
    if (uuid) {
      ...
      console.log(`Also cleared credentials for UUID: ${uuid}`)
    }
  ...
  static async setUserPassword(userId: string, email: string, password: string) {
    try {
      console.log(`Setting password for user: ${userId} (${email})`)
      ...
      if (uuid) {
        ...
        console.log(`Also stored credentials for UUID: ${uuid}`)
      }
  ...
  static async checkStoredCredentials(userId: string) {
    try {
      console.log(`Checking stored credentials for: ${userId}`)

      // Check by user ID
      const stored = localStorage.getItem(`userCredentials_${userId}`)
      if (stored) {
        console.log('Found credentials by user ID')
        ...
        console.log('No credentials found by user ID')
      }
      ...
      if (uuid) {
        ...
        if (storedByUUID) {
          console.log(`Found credentials by UUID: ${uuid}`)
        }
      }
    } catch (error) {
