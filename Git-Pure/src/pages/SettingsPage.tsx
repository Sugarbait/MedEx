...
export const SettingsPage: React.FC<SettingsPageProps> = ({ user }) => {
    ...
    const loadSettings = async () => {
      ...
      try {
        ...
        if (response.status === 'success' && response.data) {
          ...
          console.log('MFA toggle state on load:', {
            actualMFAEnabled,
            userSettings: loadedSettings.mfaEnabled,
          ...
          console.log('MFA toggle state on load (fallback):', {
            actualMFAEnabled,
            userSettings: loadedSettings.mfaEnabled,
            user_mfa_enabled: user?.mfa_enabled
          })
          ...
          if (loadedSettings.retellApiKey || loadedSettings.callAgentId || loadedSettings.smsAgentId) {
            console.log('Initializing retell service with saved credentials:', {
              hasApiKey: !!loadedSettings.retellApiKey,
              callAgentId: loadedSettings.callAgentId,
              smsAgentId: loadedSettings.smsAgentId
            })
  ...
  const handleApiKeyUpdate = async (apiKey: string) => {
    ...
    console.log('Updating API key, new credentials:', {
      hasApiKey: !!apiKey,
      callAgentId: currentSettings.callAgentId,
      smsAgentId: currentSettings.smsAgentId
    })

    // Update the retell service with new credentials
  ...
  const handleCallAgentIdUpdate = async (agentId: string) => {
    ...
    console.log('Updating Call Agent ID, new credentials:', {
      hasApiKey: !!currentSettings.retellApiKey,
      callAgentId: agentId,
      smsAgentId: currentSettings.smsAgentId
    })
  ...
  const handleSmsAgentIdUpdate = async (agentId: string) => {
    ...
    console.log('Updating SMS Agent ID, new credentials:', {
      hasApiKey: !!currentSettings.retellApiKey,
      callAgentId: currentSettings.callAgentId,
      smsAgentId: agentId
    })
  ...
  const testApiConnection = async () => {
    ...
    console.log('Testing API connection with credentials:', {
      hasApiKey: !!userSettings.retellApiKey,
      callAgentId: userSettings.callAgentId,
      smsAgentId: userSettings.smsAgentId
    })
          ...
          <div className="flex items-center gap-2">
            {syncStatus === 'synced' ? (
              ...
              <>
                ...
                <span className="text-xs text-gray-500">Sync Error</span>
              </>
            ) : null}
          </div>
        </div>
      ...
      {errorMessage && (
        <div className={`border rounded-lg p-4 flex items-start gap-3 ${
          ...
          <AlertTriangleIcon className={`w-5 h-5 flex-shrink-0 mt-0.5 ${
            syncStatus === 'offline' ? 'text-blue-600' : 'text-red-600'
          }`} />
          <div className="flex-1">
            <h4 className={`text-sm font-medium ${
              syncStatus === 'offline' ? 'text-blue-800' : 'text-red-800'
            }`}>
              {syncStatus === 'offline' ? 'Offline Mode' : 'Settings Error'}
            </h4>
            <p className={`text-sm mt-1 ${
              syncStatus === 'offline' ? 'text-blue-700' : 'text-red-700'
            }`}>
              {errorMessage}
            </p>
            {syncStatus === 'offline' && (
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              ...
              <div className="space-y-6">
                      ...
                      <div className="flex gap-3">
                        <label className="cursor-pointer">
                          <input
                            ...
                            disabled={isLoading}
                          />
                          <div className="flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors text-gray-700 text-sm font-medium">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={user?.name || 'Dr. Sarah Johnson'}
                    readOnly
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-50 text-gray-900 focus:outline-none"
                  />
                </div>
                  ...
                  <div className="flex-1">
                    ...
                    {userSettings?.mfaEnabled && (
                      <div className="flex items-center gap-2 mt-2">
                        <div className="w-2 h-2 rounded-full bg-green-500" />
                        <span className="text-xs text-green-600">MFA is properly configured</span>
                      </div>
                    )}
                  </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-blue-700">API Key</span>
                      <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-blue-700">Call Agent ID</span>
                      <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-blue-700">SMS Agent ID</span>
                      <div className="flex items-center gap-2">
          ...
          {activeTab === 'appearance' && (
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              ...
              <div className="space-y-6">
                ...
                {/* Debug Section - Cross-device Persistence Testing */}
                <div className="pt-4 border-t border-gray-200">
                  <h3 className="font-medium text-gray-900 mb-3">Debug & Testing</h3>
                  <div className="space-y-3">
                    ...
                    <div className="text-xs text-gray-500 space-y-1">
                      <p>• Use "Test Database Connection" to verify Supabase connectivity</p>
                      <p>• Use "Test Settings Persistence" to toggle theme and verify sync</p>
                      <p>• Watch browser console for detailed logging</p>
                      <p>• Check sync status indicator in header</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {isLoadingAudit ? (
                ...
                <div className="space-y-4">
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-gray-200">
                          ...
                          <th className="text-left py-2 px-3 font-medium text-gray-900">Resource</th>
                          <th className="text-left py-2 px-3 font-medium text-gray-900">PHI</th>
                          <th className="text-left py-2 px-3 font-medium text-gray-900">Outcome</th>
                        </tr>
                      </thead>
                      <tbody>
                  ...
                  <div className="bg-blue-50 rounded-lg p-4">
                    <h4 className="font-medium text-blue-900 mb-2">Audit Log Information</h4>
