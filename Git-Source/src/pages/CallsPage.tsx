...
import { retellService, type RetellCall } from '@/services'
import {
  PhoneIcon,
  PlayIcon,
  DownloadIcon,
...
export const CallsPage: React.FC<CallsPageProps> = ({ user }) => {
  ...
  // Auto-refresh functionality with increased interval to prevent rate limiting
  const { formatLastRefreshTime } = useAutoRefresh({
    enabled: true,
    interval: 120000, // 2 minutes instead of 1 to reduce API calls
    onRefresh: () => {
      fetchCalls()
  ...
  const fetchCalls = async (retryCount = 0) => {
    ...
    try {
      ...
      // Calculate metrics using ALL calls for accurate totals
      const calculatedMetrics = retellService.calculateCallMetrics(totalCalls)
      ...
      console.log('📊 Calculated Metrics:', {
        ...
        rawCallCosts: totalCalls.map(c => ({
          id: c.call_id,
          cost_cents: c.call_cost?.combined_cost,
          cost_dollars: c.call_cost?.combined_cost ? (c.call_cost.combined_cost / 100).toFixed(4) : 0
        }))
      })
  ...
  const formatCallCost = (call: Call) => {
    // Use the proper Retell API cost structure
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="text-xs text-gray-500 mb-6">
        Last refreshed: {formatLastRefreshTime()} (Auto-refresh every 2 minutes)
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          ...
          <div className="text-3xl font-black text-blue-600 mb-1">
            ${loading ? '...' : metrics.avgCostPerCall.toFixed(2)}
          </div>
          <div className="text-xs text-gray-500">
            Total cost: ${(metrics.totalCost || 0).toFixed(2)}
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          ...
          <div className="text-3xl font-black text-blue-600 mb-1">
            ${loading ? '...' : metrics.highestCostCall.toFixed(2)}
          </div>
          <div className="text-xs text-gray-500">
            Lowest: ${metrics.highestCostCall.toFixed(2)}
          </div>
        </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="text-right">
            <div className="text-3xl font-black text-green-600">${loading ? '...' : (metrics.totalCost || 0).toFixed(2)}</div>
          ...
          <div className="mb-8 bg-white border border-blue-200 rounded-xl p-6 shadow-sm">
            <div className="flex justify-between items-center mb-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-gray-900">Active Call</h3>
                  <p className="text-gray-600">Status: {callStatus}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <button
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <div className="h-32 overflow-y-auto bg-white p-4 rounded border text-sm">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="text-center py-16">
              ...
              <h3 className="text-xl font-medium text-gray-900 mb-2">No calls found</h3>
